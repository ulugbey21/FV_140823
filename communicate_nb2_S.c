...
void communicate_nb2_S (RUN * run,int ifield) {
  ...
  adr=malloc(10*sizeof(int));
	...
	for (ipr=0;ipr<run->con->size;ipr++){ // loop proxies to receive
		...
		if (run->topo->nprox[ipr]!=0) {
			...
			if (ifield<3){ 
				MPI_Isend(sendbuff,ibufsize,MPI_DOUBLE,ipr,run->con->rank,MPI_COMM_WORLD,&request);
			} else { 
				MPI_Isend(isendbuff,ibufsize,MPI_INT,ipr,run->con->rank,MPI_COMM_WORLD,&request);
			}
			if (ifield<3){ 
				free(sendbuff);
			} else { 
  ...
  for (ipt=0;ipt<run->con->size;ipt++){ // loop of proxies to send
      ...
      if (ifield<3){ 
        MPI_Irecv(recvbuff,ibufsize,MPI_DOUBLE,ipt,ipt,MPI_COMM_WORLD,&request);
				MPI_Wait(&request, MPI_STATUS_IGNORE);
      } else { 
        MPI_Irecv(irecvbuff,ibufsize,MPI_INT,ipt,ipt,MPI_COMM_WORLD,&request);
				MPI_Wait(&request, MPI_STATUS_IGNORE);
      }
